...
> 最後更新：2026-05-29（v9.14 動態速度上限系統完整化 + 效能自適應 + 互動回饋，3 組共 11 項）
...
### 🎯 v9.14：動態速度上限系統完整化 + 效能自適應 + 互動回饋（最新完成）

> 最後更新：2026-05-29（承接 v9.12，3 組共 11 項；commit `e83b0b5` / `815e9c4` / `fbc92d0`）

**🔧 速度上限系統「閉環」（BW/BX/BY/BZ/CA/CF）：**
- [x] **BW 補最大破口**：放手後接手的持續自轉速度（原固定 1.5°/幀）與慣性甩動初速，改收斂到動態 `_readonlyMaxOmega`；跟手階段（`handleDragMove`）刻意不動，保留 1:1 跟手手感。
- [x] **抽出 `_recomputeSpeedCap()` 單一真相來源**：進場 / resize / 縮放三處共用同一套反推公式，避免散落多份難維護。
- [x] **BX resize / 螢幕旋轉重算**：`innerWidth` 變 → 目標視覺速度變，debounce 200ms 重算。
- [x] **BY pinch / wheel 縮放後重算**：`perspective` 變 → `R_proj` 變 → 重算上限。
- [x] **BZ 多排取最外排半徑**：`_maxRowRadius` 取 cols 最大那排，多排相簿最外排不再飛快。
- [x] **CA 進場期間鎖拖曳**：`_introLocked` 在 1800ms 推近動畫期間擋拖曳起手，含 2.2s 保險解鎖。
- [x] **CF 進場起點自適應**：`START` 依張數調整，小相簿不從超遠 8000 空蕩推進。
- [x] **下限保護 0.02**：防 in-app webview 初期 `innerWidth=0` 時自轉被算成 0 鎖死。

**⚡ 效能與裝置自適應（CB/CC/CD）：**
- [x] **CB FPS 實測動態降級**：`applyTransform` 掛 `_perfSample` 輕量採樣，每 1.5s 評估，連續 <40fps 升 `_perfTier`（關桌機粒子 → 光照跳幀 2→3），夠順或降到底自動停止採樣。
- [x] **CC 低端裝置預判**：`navigator.deviceMemory≤4` 或 `hardwareConcurrency≤4` → 第一幀就關粒子、`CHUNK_SIZE` 降 4、預設降一級。
- [x] **CD prefers-reduced-motion**：尊重系統「減少動態」→ 不自動旋轉、跳過進場推近與波浪入場，並加 `@media` 關閉卡片動畫。

**👁️ 互動回饋（CE/CG）：**
- [x] **CE 互動回饋 HUD**：`flashHUD()` 在點背景暫停/繼續時顯示「▶ 旋轉中 / ⏸ 已暫停」+ `navigator.vibrate` 震動回饋。
- [x] **CG 按住快轉**：唯讀模式右下浮動鈕，按住加速（動態上限 6 倍封頂 2°/幀）、放開呼叫 `_recomputeSpeedCap` 還原，給「想快速掃完」的出口。

> Preview 驗證：頁面正常載入、零 JS 錯誤、touchSens 反推 43張→0.143 / 8張→0.5、FPS 採樣與 CG 加速/還原邏輯實測正確。

---

### 🎯 v9.12：分享相簿手機旋轉速度動態自適應
...
- [x] **卡片翻轉瞬切**：`.carousel-item-inner` 的 0.65s `transition` 在手機端改為 `none`，移除觸發 repaint 的動畫。
- [x] **跳過 box-shadow 動態計算**：`applyTransform()` 手機端只寫 `brightness()`，完全跳過每幀 `boxShadow` DOM 寫入，消除 GPU repaint 開銷。

### 🗂️ v9.7：多維度排序選項
- [x] **功能 AF** 版型抽屜新增「🗂️ 排序方式」下拉選單：自訂順序 / 日期新→舊 / 日期舊→新 / 名稱 A→Z / 名稱 Z→A / 標籤分類。
- [x] **排序偏好記憶**：排序設定儲存至 localStorage，重整後自動還原。
- [x] **全路徑套用**：標籤篩選、上傳完成、刪除、雲端還原、初始載入均套用當前排序；拖曳排序時例外（不覆蓋手動順序）。
...
| **v9.14** | **BW–CG** | **速度上限系統完整化 + 效能自適應(FPS/低端/a11y) + 互動回饋HUD/快轉（11 項）** | **⭐⭐** | **⭐⭐** | **⭐⭐⭐⭐⭐** | **✅ 已完成** |
...
## ✅ v9.14：動態速度上限系統「完整化」+ 效能自適應（已全部實作）

> **✅ 本批 11 項已全部實作完成**（commit `e83b0b5` 閉環 / `815e9c4` 效能 / `fbc92d0` 體驗，皆通過 preview 驗證）。下方保留設計脈絡與程式碼供日後維護參考。
>
> 本批**直接延續 v9.12「分享相簿手機旋轉速度動態自適應」**。v9.12 已讓「自動旋轉初速」依環形投影半徑 `R_proj` 收斂，但這套系統還有幾個**未閉環的缺口**——`_maxOmegaDeg` 只在 `_readonlyZoomInAnimation()` 進場時算一次、只套到 `autoRotateBase / autoRotateSpeed / touchSens`，一旦使用者**手動互動**或**改變視窗 / 視角**就會被繞過。以下按「① 速度系統閉環 → ② 效能/裝置自適應 → ③ 觀看體驗」三組排列，每項附**與 v9.12 的關係**、程式碼示意與效益。
...
> ✅ 實際實作順序：閉環組（BW–CF + 下限保護）→ 效能組（CB/CC/CD）→ 體驗組（CE/CG），分 3 個 commit 完成並通過 preview 驗證。
